...
(function(window, doc) {
    var littleCharts = doc.getElementsByClassName('little-chart');
    var isElementInViewport = function(el) {
        var rect = el.getBoundingClientRect();
        var height = rect.bottom - rect.top;
        var width = rect.right - rect.left;
        return (rect.top >= -height && rect.left >= -width && rect.bottom <= (window.innerHeight || doc.documentElement.clientHeight) + height && rect.right <= (window.innerWidth || doc.documentElement.clientWidth) + width);
    };
    var renderLittleCharts = function() {
        ...
        for (var i = 0, l = littleCharts.length; i < l; i++) {
            var littleChart = littleCharts[i];
            var littleCtx = littleChart.getContext('2d');
            var loaded = littleChart.getAttribute('data-loaded');
            var dataMedia = littleChart.getAttribute('data-media');
            if (!isElementInViewport(littleChart) && loaded === 'loaded') {
                littleCtx.clearRect(0, 0, littleChart.width, littleChart.height);
                littleChart.setAttribute('data-loaded', '');
                continue;
            }
            if (!isElementInViewport(littleChart)) continue;
            if (loaded === 'loaded' && dataMedia === media) continue;
            (function(littleChart, littleCtx) {
                setTimeout(function() {
                    littleChart.setAttribute('data-loaded', 'loaded');
                    littleChart.setAttribute('data-media', media);
                    var fullWidth = width * window.devicePixelRatio;
                    var fullHeight = height * window.devicePixelRatio;
                    var canvas = doc.createElement('canvas');
                    var ctx = canvas.getContext('2d');
                    canvas.width = String(fullWidth);
                    canvas.height = String(fullHeight);
                    canvas.style.width = width + 'px';
                    canvas.style.height = height + 'px';
                    var dataPoints = littleChart.getAttribute('data-points');
                    var splitPoints = dataPoints.split(',');
                    var points = splitPoints.map(function(point) {
                        return Number(point);
                    });
                    ...
                    ctx.beginPath();
                    if (min === max) {
                        ctx.moveTo(lineWidth, fullHeight / 2);
                        ctx.lineTo((fullWidth - (lineWidth * 2)), fullHeight / 2);
                    } else {
                        for (var i = 0, l = points.length; i < l; i++) {
                            var x = lineWidth + (i * ((fullWidth - (lineWidth * 2)) / (l - 1)));
                            var y = lineWidth + ((max - points[i]) * ((fullHeight - (lineWidth * 2)) / (max - min)));
                            if (i === 0) {
                                ctx.moveTo(x, y);
                            } else {
                                ctx.lineTo(x, y);
                    ...
                    if (littleChart.classList.contains('little-chart--negative')) {
                        ctx.strokeStyle = 'rgba(255,96,96,0.8)';
                    } else {
                        ctx.strokeStyle = 'rgba(60,212,131,0.8)';
                    }
                    ctx.lineWidth = lineWidth;
                    ctx.stroke();
                    littleChart.width = canvas.width;
                    littleChart.height = canvas.height;
                    littleChart.style.width = canvas.style.width;
                    littleChart.style.height = canvas.style.height;
                    var opacity = 0;
                    (function fadeIn() {
                        littleCtx.globalAlpha = opacity;
                        littleCtx.clearRect(0, 0, littleChart.width, littleChart.height);
                        littleCtx.drawImage(canvas, 0, 0);
                        opacity += 0.08;
                        if (opacity < 1) {
            ...
            })(littleChart, littleCtx);
        }
    };
    window.addEventListener('scroll', renderLittleCharts);
    window.addEventListener('resize', renderLittleCharts);
    renderLittleCharts();
    var jsHide = doc.querySelectorAll('.js-hide');
    for (var i = 0, l = jsHide.length; i < l; i++) {
        jsHide[i].style.display = 'none';
    }
    /*
    var changeBaseCurrency = doc.querySelector('[data-action="change-base-currency"]');
    if (changeBaseCurrency !== null) {
        changeBaseCurrency.addEventListener('change', function(e) {
            e.preventDefault();
            this.form.submit();
        });
    }
    */
    var base_symbol = function(base) {
        switch (base) {
            case 'AUD':
                return '$';
            case 'BGN':
    ...
    var backButtonToAllCoins = doc.querySelector('[data-action="back-to-all-coins"]'),
        stripQueryStringFromURL = function(url) {
            return url.split('?')[0];
        };
    if (backButtonToAllCoins !== null) {
        backButtonToAllCoins.addEventListener('click', function(e) {
            var href = this.href;
            if (stripQueryStringFromURL(doc.referrer) === href) {
                e.preventDefault();
                return history.go(-1);
            }
        });
    }
    var coinsListBody = doc.querySelector('.coin-list__body');
    var coinsListFooter = doc.querySelector('.coin-list__footer');
    var coinRows = doc.getElementsByClassName('coin-list__body__row');
    var backToTopElm = doc.querySelector('[data-action="coin-list--back-to-top"]');
    if (backToTopElm !== null) {
        backToTopElm.addEventListener('click', function(e) {
            e.preventDefault();
            window.scrollTo(0, 0);
        });
    }
    if (Stickyfill) {
        var stickyHeader = doc.querySelector('.coin-list__header');
        if (stickyHeader !== null) {
            Stickyfill.add(stickyHeader);
        }
        var proccessHeader = doc.querySelector('.exchange__process');
        if (proccessHeader !== null) {
            Stickyfill.add(proccessHeader);
        }
    }
    ...
    var coin_number_format = function(value, default_decimal_amount, method) {
        var decimal_point = '.';
        var thousands_separator = ',';
        var price = Number(value);
        var decimal_count = default_decimal_amount;
        if (price === 0) {
            return 0;
        }
        if (price <= 0.000001) {
            decimal_count = 9;
        } else if (price.toFixed(3) < 1) {
            var decimal_zeros = -Math.floor(Math.log(price) / Math.log(10) + 1);
            if (decimal_zeros === -0) {
                decimal_count = 3;
            } else {
                decimal_count = decimal_zeros + 3;
            }
        }
        rounded_price = price;
        switch (method) {
            case 'ceil':
                rounded_price = Math.ceil(price * Math.pow(10, decimal_count)) / Math.pow(10, decimal_count);
                break;
            case 'floor':
                rounded_price = Math.floor(price * Math.pow(10, decimal_count)) / Math.pow(10, decimal_count);
                break;
        }
        return number_format(rounded_price, decimal_count, decimal_point, thousands_separator);
    };
    var anyHasSameResult = function(elements, filter) {
        var filteredElements = [];
        for (var i = 0, l = elements.length; i < l; i++) {
            var element = elements[i],
                filteredElement = filter(element);
            if (filteredElements.indexOf(filteredElement) !== -1) {
                return true;
            }
    ...
    var tooltipMeta = doc.createElement('div');
    tooltipMeta.className = 'ct-tooltip-meta';
    var tooltipValue = doc.createElement('div');
    tooltipValue.className = 'ct-tooltip-value';
    var activeHitBox = null;
    var activePoint = null;
    var hideToolTip = function() {
        if (activeHitBox) {
            activeHitBox.setAttribute('class', 'ct-hit-box');
            activeHitBox = null;
        }
        if (activePoint) {
            activePoint.setAttribute('class', 'ct-point');
            activePoint = null;
        }
        tooltip.className = 'ct-tooltip';
    };
    var theChart = null;
    var chartError = function(errMessage) {
        if (theChart) {
            theChart.destroy();
            theChart = null;
        }
        var canvas = doc.querySelector('.coin-page__chart');
        var ctx = canvas.getContext('2d');
        var parent = canvas.parentNode;
        var computedStyle = getComputedStyle(parent);
        var parentWidth = parent.clientWidth;
        var parentHeight = parent.clientHeight;
        canvas.width = parentWidth - (parseFloat(computedStyle.paddingLeft) + parseFloat(computedStyle.paddingRight));
        canvas.height = parentHeight - (parseFloat(computedStyle.paddingTop) + parseFloat(computedStyle.paddingBottom));
        ctx.fillStyle = "#7289a1";
        ctx.font = '20px Montserrat';
        var textWidth = ctx.measureText(errMessage).width;
        ctx.fillText(errMessage, (canvas.width / 2) - (textWidth / 2), (canvas.height / 2) - 10);
    };
    var updateChart = function(chart, data) {
        hideCustomTooltip();
        chart.options.scales.xAxes[0].time.min = data.history[0].timestamp;
        chart.options.scales.xAxes[0].time.max = Date.now();
        chart.data.labels = data.history.map(function(item) {
            return item.timestamp;
        });
        var dataset = chart.data.datasets[0];
        dataset.data = data.history.map(function(item) {
            return item.price;
        });
        chart.update(0);
        return chart;
    };
    var createChart = function(chart, data) {
        var chartSwiped = false;
        var chartStartPointX = 0;
        chart.addEventListener('touchstart', function(e) {
            var point = e.touches ? e.touches[0] : e;
            chartSwiped = false;
            chartStartPointX = point.pageX;
        });
        chart.addEventListener('touchmove', function(e) {
            ...
            var point = e.touches ? e.touches[0] : e;
            if (chartStartPointX - point.pageX > 20) {
                chartSwiped = true;
                e.preventDefault();
            }
        });
        var ctx = chart.getContext('2d');
        var yAxesCallbackMobile = function(value, index, values) {
            if (index === 0 || index === values.length - 1) {
                return base_symbol(variables.base) + ' ' + coin_number_format(value, 2);
            } else {
                return '';
            }
        };
        var yAxesCallbackDesktop = function(value, index, values) {
            return base_symbol(variables.base) + ' ' + coin_number_format(value, 2);
        };
        var options = {
            responsive: true,
            ...
            tooltips: {
                ...
                intersect: false,
                mode: 'index',
                custom: customTooltip.bind(chart)
            },
            layout: {
                padding: {
                    left: 0,
            ...
            scales: {
                ...
                yAxes: [{
                    ...
                    gridLines: {
                        color: 'rgba(28, 62, 98, 0.6)',
                        zeroLineColor: 'rgba(28, 62, 98, 0.6)',
                        drawBorder: false,
                        tickMarkLength: 0
                    },
                    ticks: {
            ...
            onResize: function(chart, size) {
                ...
                if (window.innerWidth > 567) {
                    ...
                    chart.options.scales.yAxes[0].ticks.callback = yAxesCallbackDesktop;
                    chart.options.scales.yAxes[0].gridLines.tickMarkLength = 20;
                    chart.options.layout.padding.left = 0;
                    chart.options.layout.padding.right = 0;
                } else {
                    chart.options.scales.yAxes[0].ticks.mirror = true;
                    chart.options.scales.yAxes[0].ticks.callback = yAxesCallbackMobile;
                    chart.options.scales.yAxes[0].gridLines.tickMarkLength = 0;
                    chart.options.layout.padding.left = 10;
                    chart.options.layout.padding.right = 10;
                }
                var gradient = ctx.createLinearGradient(0, 0, 0, (chart.height / window.devicePixelRatio) - 38);
                gradient.addColorStop(0, 'rgba(96,172,243,0.2)');
                gradient.addColorStop(1, 'rgba(96,172,243,0)');
                chart.data.datasets[0].backgroundColor = gradient;
                chart.update(0);
            }
        };
        if (window.innerWidth > 567) {
            options.scales.yAxes[0].ticks.mirror = false;
            options.scales.yAxes[0].ticks.callback = yAxesCallbackDesktop;
            options.scales.yAxes[0].gridLines.tickMarkLength = 20;
            options.layout.padding.left = 0;
            options.layout.padding.right = 0;
        } else {
            options.scales.yAxes[0].ticks.mirror = true;
            options.scales.yAxes[0].ticks.callback = yAxesCallbackMobile;
            options.scales.yAxes[0].gridLines.tickMarkLength = 0;
            options.layout.padding.left = 10;
            options.layout.padding.right = 10;
        }
        var thisChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    pointBorderColor: '#60acf3',
                    pointHoverBorderColor: '#60acf3',
                    pointBorderWidth: 0,
        ...
        var gradient = ctx.createLinearGradient(0, 0, 0, (ctx.canvas.height / window.devicePixelRatio) - 38);
        gradient.addColorStop(0, 'rgba(96,172,243,0.2)');
        gradient.addColorStop(1, 'rgba(96,172,243,0)');
        thisChart.data.datasets[0].backgroundColor = gradient;
        thisChart.update();
        return thisChart;
    };
    var hideCustomTooltip = function() {
        var tooltipEl = doc.querySelector('.tooltip');
        var tooltipCursorEl = doc.querySelector('.tooltip-cursor');
        var tooltipPointEl = doc.querySelector('.tooltip-point');
        tooltipEl.style.opacity = 0;
        tooltipCursorEl.style.opacity = 0;
        tooltipPointEl.style.opacity = 0;
        tooltipEl.style.left = '0px';
        tooltipEl.style.top = '0px';
        tooltipCursorEl.style.top = '0px';
        tooltipCursorEl.style.height = '0px';
        tooltipCursorEl.style.left = '0px';
        tooltipPointEl.style.left = '0px';
        tooltipPointEl.style.top = '0px';
    };
    var customTooltip = function(tooltip) {
        ...
        if (tooltip.body) {
            ...
            var innerHtml = '';
            innerHtml += '<div class="tooltip__title">' + formatDate(titleLines[0]) + '</div>';
            innerHtml += '<div class="tooltip__body">' + base_symbol(variables.base) + ' ' + coin_number_format(bodyLines[0][0], 2) + '</div>';
            tooltipEl.innerHTML = innerHtml;
        }
        var position = this.getBoundingClientRect();
        tooltipEl.style.opacity = 1;
        var tooltipLeft = (position.left + tooltip.caretX - (tooltipEl.offsetWidth / 2))
        if (tooltipLeft < 0) {
            tooltipLeft = 0;
        }
        if (tooltipLeft + tooltipEl.offsetWidth > window.innerWidth) {
            tooltipLeft = window.innerWidth - tooltipEl.offsetWidth;
        }
        var scrollTop = (doc.documentElement.scrollTop || doc.body.scrollTop);
        tooltipEl.style.left = tooltipLeft + 'px';
        tooltipEl.style.top = (position.top - tooltipEl.offsetHeight + scrollTop + 7) + 'px';
        tooltipCursorEl.style.opacity = 1;
        tooltipCursorEl.style.top = (position.top + scrollTop + 7) + 'px';
        tooltipCursorEl.style.height = (position.height - 31) + 'px';
        tooltipCursorEl.style.left = (position.left + tooltip.caretX) + 'px';
        tooltipPointEl.style.opacity = 1;
        tooltipPointEl.style.left = (position.left + tooltip.caretX - 3) + 'px';
        tooltipPointEl.style.top = (position.top + tooltip.caretY + scrollTop - (tooltipPointEl.offsetHeight / 2)) + 'px';
    };
    var getChartData = function(id, timeframe, callback) {
        var xhr = new XMLHttpRequest();
        xhr.open('GET', variables.api + '/public/coin/' + id + '/history/' + timeframe + '?base=' + variables.base);
        xhr.addEventListener('load', function() {
            ...
            if (response.status !== 'success' || response.data.history.length === 0) {
    ...
    var chart = doc.querySelector('.coin-page__chart');
    var spinner = doc.querySelector('.coin-page__title-spinner');
    var change = doc.querySelector('.coin-page__title-change');
    if (chart !== null) {
        var data_time_frame = chart.getAttribute('data-timeframe'),
            data_id = chart.getAttribute('data-id');
        spinner.classList.remove('coin-page__title-spinner--hidden');
        change.classList.add('coin-page__title-change--hidden');
        change.classList.remove('coin-page__title-change--positive');
        change.classList.remove('coin-page__title-change--negative');
        getChartData(data_id, data_time_frame, function(err, data) {
            spinner.classList.add('coin-page__title-spinner--hidden');
            if (err) return chartError('Could\'t find any historical data');
            var firstPrice = parseFloat(data.history[0].price);
            if (firstPrice !== 0) {
                change.classList.remove('coin-page__title-change--hidden');
                var changePercentage = parseFloat(data.change);
                var displayChange = changePercentage;
                var arrow = '';
                if (changePercentage > 0) {
                    change.classList.add('coin-page__title-change--positive');
                    arrow = '<img class="coin-page__title-change-icon coin-page__title-change-icon--up" src="' + variables.url + '/static/img/arrow-up-green.svg" width="5" height="7">';
                } else if (changePercentage < 0) {
                    change.classList.add('coin-page__title-change--negative');
                    displayChange = Math.abs(changePercentage);
                    arrow = '<img class="coin-page__title-change-icon coin-page__title-change-icon--down" src="' + variables.url + '/static/img/arrow-down-red.svg" width="5" height="7">';
                }
                change.innerHTML = number_format(displayChange, 2, '.', ',') + '%' + arrow;
            }
            theChart = createChart(chart, data);
        });
    }
    var changeTimeFrameButton = doc.querySelectorAll('[data-action="change-time-frame"]');
    for (var i = 0, l = changeTimeFrameButton.length; i < l; i++) {
        changeTimeFrameButton[i].addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            spinner.classList.remove('coin-page__title-spinner--hidden');
            change.classList.add('coin-page__title-change--hidden');
            change.classList.remove('coin-page__title-change--positive');
            change.classList.remove('coin-page__title-change--negative');
            var data_time_frame = this.getAttribute('data-time-frame'),
                data_id = this.getAttribute('data-id'),
                data_name = this.getAttribute('data-name');
            var activeButton = doc.querySelector('.button--blue[data-action="change-time-frame"]');
            if (activeButton !== null) {
                activeButton.classList.remove('button--blue');
                activeButton.classList.add('button--dark');
            }
            ...
            getChartData(data_id, data_time_frame, function(err, data) {
                ...
                if (theChart !== null) {
                    theChart = updateChart(theChart, data);
                } else {
                    var chart = doc.querySelector('.coin-page__chart');
                    theChart = createChart(chart, data);
                }
    ...
    var dontShowFocusChrome = function() {
        this.blur();
    };
    if (isChrome) {
        var aElms = doc.getElementsByTagName('a');
        for (var i = 0, l = aElms.length; i < l; i++) {
            aElms[i].addEventListener('mousedown', dontShowFocusChrome);
            aElms[i].addEventListener('mouseup', dontShowFocusChrome);
        }
        var buttonElms = doc.getElementsByTagName('button');
        for (var i = 0, l = buttonElms.length; i < l; i++) {
            buttonElms[i].addEventListener('mousedown', dontShowFocusChrome);
            buttonElms[i].addEventListener('mouseup', dontShowFocusChrome);
        }
    }
    var searchElm = doc.querySelector('.search-container');
    var searchLabel = doc.querySelector('.search__label');
    var searchClose = doc.querySelector('.search__close');
    var searchField = doc.querySelector('.search__field');
    if (searchField !== null) {
        searchField.addEventListener('focus', function(e) {
            searchElm.classList.remove('search--field-hidden');
        });
    }
    if (searchLabel !== null) {
    ...
    if (searchClose !== null) {
        searchClose.addEventListener('click', function(e) {
            ...
            searchField.value = '';
            searchField.blur();
            searchElm.classList.add('search--field-hidden');
            search.bind(searchField)();
        });
    }
    var prevValue = variables.search;
    var updateQueryStringParameter = function(uri, queries) {
        for (var i = 0, l = queries.length; i < l; i++) {
            ...
            if (uri.match(re)) {
                if (query.value === '') {
                    ...
                    uri = uri.replace(re, '$1' + query.key + '=' + query.value + '$2');
                }
            } else {
                if (query.value !== '') {
                    uri = uri + separator + query.key + '=' + query.value;
                }
            }
        }
        return uri;
    ...
    var search = function() {
        ...
        if (value === prevValue || searching === true) return;
        prevValue = value;
        searching = false;
        var descTitles = doc.querySelectorAll('.coin-list__header__title--sorted-desc');
        var ascTitles = doc.querySelectorAll('.coin-list__header__title--sorted-asc');
        for (var i = 0, l = descTitles.length; i < l; i++) {
            descTitles[i].classList.remove('coin-list__header__title--sorted-desc');
        }
        for (var i = 0, l = ascTitles.length; i < l; i++) {
            ascTitles[i].classList.remove('coin-list__header__title--sorted-asc');
        }
        var xhr = new XMLHttpRequest();
        var limit = 50;
        var sorton = variables.sorton;
        var sortby = variables.sortby;
        var prefix = value;
        var offset = 0;
        var title = 'Coinranking - Cryptocurrency prices';
        if (prefix !== '') {
            title = prefix + ' - Coinranking';
        }
        doc.title = title;
        window.history.replaceState({}, title, updateQueryStringParameter(doc.location.href, [{
            key: 'search',
            value: value
        }, {
            key: 'page',
        ...
        /*
        xhr.open('GET', variables.url + '/api/coins?prefix=' + prefix + '&offset=' + offset + '&limit=' + limit + '&sorton=' + sorton + '&sortby=' + sortby + '&base=' + variables.base);
        xhr.addEventListener('load', function() {
            var response;
            try {
                response = JSON.parse(this.response);
            ...
            coinsListBody.innerHTML = response.data.coins;
            renderLittleCharts();
            if (prefix !== '' || response.stats.offset + response.stats.limit >= response.stats.total) {
                coinsListFooter.classList.add('coin-list__footer--hidden');
            } else {
                coinsListFooter.classList.remove('coin-list__footer--hidden');
            }
            searching = false;
            if (isChrome) {
                var aElms = doc.getElementsByTagName('a');
    ...
    if (searchField !== null) {
        ...
        searchField.addEventListener('paste', search);
    ...
    if (('standalone' in window.navigator) && window.navigator.standalone) {
        ...
        doc.addEventListener('click', function(event) {
            ...
            if ('href' in noddy && noddy.href.indexOf('http') !== -1 && (noddy.href.indexOf(doc.location.host) !== -1 || remotes)) {
